		...
		if equipped_gun == "shotgun" {
			...
			for (var _i=0; _i<_bullet_per_shell; _i++) {
				while (_range_increment < bullet_range.shotgun) {
					var _target = collision_line(x, _y, x + lengthdir_x(_range_increment, _bullet_direction - (_bullet_spread/_bullet_per_shell * _i)), _y + lengthdir_y(_range_increment, _bullet_direction - (_bullet_spread/_bullet_per_shell * _i)), obj_enemy, true, true);
					if _target {
						hit_enemy(_target);
						draw_bullets = true;
						array_push(bullets, [x, _y, x + lengthdir_x(_range_increment, _bullet_direction - (_bullet_spread/_bullet_per_shell * _i)), _y + lengthdir_y(_range_increment, _bullet_direction - (_bullet_spread/_bullet_per_shell * _i))]);
						break;
					} else { _range_increment += 10; }
				}
				draw_bullets = true;
